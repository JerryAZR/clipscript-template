...
Each narration line gets its own mp3 at `public/<episode>/voiceover/<lineId>.mp3`, generated by:

```bash
npx tsx scripts/tts.mts --episode <name> [--voice en-US-AriaNeural] [--rate +0%]
```

The script uses edge-tts (free, no API key) and is content-hash cached - only changed lines are re-synthesized. When a line's mp3 exists, `calculateMetadata` measures its real duration with Mediabunny and plays it back; without it, the line gets an estimated duration and stays silent, so episodes render fine before TTS runs. Re-run the script whenever `narration.toml` changes.

The default voice is fine for drafts. When quality matters, replace `scripts/tts.mts` with your own pipeline (e.g. an OpenAI-compatible TTS API) writing the same files - the engine only cares about the mp3 paths.
