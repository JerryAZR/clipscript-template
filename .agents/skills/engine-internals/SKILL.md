...
Module map: `src/engine/` - `types.ts` (clip defs, `ClipCommon` mixin + union), `narration.ts`, `audio.ts`, `timeline.ts`, `clips/code-state.ts`, `ClipRenderer.tsx`, `calculate-metadata.ts`, `highlight.ts`, `CodeTransition.tsx`, `Episode.tsx`, contexts (`theme`, `HighlightContext`, `EpisodeNameContext`), `clips/` (components), `code-style.ts`.
